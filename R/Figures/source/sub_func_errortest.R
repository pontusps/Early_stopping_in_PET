...
sub_func_errortest <- function(start_n_comparisons, max_n_comparisons, test_seq, k_BF01, k_BF10) {
  ...
  if (start_n_comparisons>max_n_comparisons){
    stop('"start_n_comparisons" must be lower than "max_n_comparisons"')
  }
  
  #Test if "test_seq" is to high
  if (test_seq>(max_n_comparisons-start_n_comparisons)){
    stop('"test_seq" is set too high')
  }
  
  if (start_n_comparisons<2){
    stop('"start_n_comparisons must be set to two or higher"')
  }
