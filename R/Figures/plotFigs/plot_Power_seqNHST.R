##########################################################
# "Power" graphs for seq-BF, seq-NHST and fixed N NHST   #
##########################################################
### Pontus Plavén-Sigray (NRU) & Jonas Svensson, KI, October 2020
...
plot_Power_seqNHST <- function(path, 
                       paired,
  ...
  #specify colors for lines to draw
  color_input <- c( "#059af4", "black", "#E69F00" )
  linetype_input <- c("dashed","dashed","dashed")
  
  #########################
  # First plot: "power" curve
  #########################
  
  p1 <- ggplot(data = df_to_plot,aes(x = D, y = Decision, group = outcome))+
    geom_line(size=2, aes(linetype=outcome, color = outcome))+
  ...
  p2 <- ggplot(data = df_to_plot,
               aes(x = D, y = N, group = outcome))+
    geom_line(size = 2, aes(linetype=outcome, color = outcome) )+
